...
export default function ArticleDetailPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
      ...
      {/* ── パンくず ── */}
      <nav className="flex items-center gap-1 text-[12px] sm:text-[11px] text-gray-400 mb-4 animate-fade-in">
      ...
      <header className="mb-5 animate-fade-in-up">
        ...
        <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5 mb-3">
          <span className={`inline-flex items-center gap-1 text-[12px] sm:text-[11px] font-medium px-1.5 py-0.5 rounded ${style.badge}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${style.dot}`} />
            {article.category}
          </span>
          {article.featured && (
            <span className="text-[12px] sm:text-[11px] font-medium px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-600 ring-1 ring-amber-200/50">注目</span>
          )}
          <span className="text-[12px] sm:text-[11px] text-gray-400">{article.publishedAt}</span>
        ...
        <a
          ...
          className="inline-flex items-center gap-1 text-[13px] sm:text-[12px] text-primary-600 hover:text-primary-700 bg-primary-50 px-2.5 py-1 rounded-md transition-colors"
        >
      ...
      <section className="mb-6 animate-fade-in-up stagger-1">
        ...
        <div className="bg-white rounded-lg border border-gray-200/80 p-4">
          <p className="text-[15px] sm:text-[15px] text-gray-700 leading-[1.85] whitespace-pre-wrap">
            {article.summary}
          </p>
        </div>
      </section>
            ...
            <ul className="space-y-2.5">
              {article.actionItems.map((item, i) => (
                <li key={i} className="flex items-start gap-2.5">
                  ...
                  <span className="text-[15px] sm:text-[15px] text-gray-700 leading-relaxed">{item}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      {(article.relatedThemes.length > 0 || article.tags.length > 0) && (
        <section className="mb-6 animate-fade-in-up stagger-4">
          {article.relatedThemes.length > 0 && (
            <div className="mb-3">
              <h2 className="text-[13px] sm:text-[12px] font-bold text-gray-500 mb-2">関連テーマ</h2>
              <div className="flex flex-wrap gap-1.5">
                {article.relatedThemes.map((theme) => (
                  <span key={theme} className="text-[13px] sm:text-[12px] bg-primary-50 text-primary-700 px-2.5 py-0.5 rounded-md font-medium">
                    {theme}
                  </span>
                ))}
              </div>
          ...
          {article.tags.length > 0 && (
            <div>
              <h2 className="text-[13px] sm:text-[12px] font-bold text-gray-500 mb-2">タグ</h2>
              <div className="flex flex-wrap gap-1.5">
                {article.tags.map((tag) => (
                  <span key={tag} className="text-[13px] sm:text-[12px] bg-gray-100 text-gray-500 px-2.5 py-0.5 rounded-md">
                    {tag}
                  </span>
                ))}
              </div>
      ...
      {related.length > 0 && (
        <section className="mb-6 animate-fade-in-up stagger-5">
          <h2 className="text-[13px] sm:text-[12px] font-bold text-gray-500 mb-2.5">関連する記事</h2>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[14px] sm:text-[13px] font-bold text-gray-800 group-hover:text-primary-700 transition-colors leading-snug">
                      {a.title}
                    </p>
                    <p className="text-[12px] sm:text-[11px] text-gray-400 mt-0.5">
      ...
      <div className="pt-4 border-t border-gray-100 animate-fade-in-up stagger-6">
        <Link
          to="/articles"
          className="inline-flex items-center gap-1 text-[14px] sm:text-[13px] text-primary-600 hover:text-primary-700 no-underline font-medium"
        >
