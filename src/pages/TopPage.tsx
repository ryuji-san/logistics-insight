...
export default function TopPage() {
  ...
  // トレンド記事からアクションを集約
  const topActions = trending
    .flatMap((a) =>
      (a.actionItems ?? []).slice(0, 1).map((item) => ({ item, articleId: a.id, articleTitle: a.title }))
    )
    .slice(0, 5);
    ...
    <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
      ...
      <section className="mb-5 sm:mb-8 animate-fade-in-up">
        <div className="rounded-lg bg-primary-700 hero-pattern p-4 sm:p-6 text-white relative overflow-hidden">
          ...
          <p className="text-primary-200 text-[12px] mb-0.5">{getGreeting()}</p>
          <h1 className="text-base sm:text-lg font-bold mb-1">物流インサイト</h1>
          <p className="text-primary-100 text-[12px] sm:text-[13px] leading-relaxed max-w-md">
            物流政策・業界動向の要点と示唆をまとめて共有。現場の高度化・DX検討にお役立てください。
          </p>
          <div className="mt-3 flex flex-wrap items-center gap-2.5">
            <Link
              to="/articles"
              className="inline-flex items-center gap-1 bg-white text-primary-700 font-semibold text-[12px] sm:text-[13px] px-3.5 py-1.5 rounded-md hover:bg-primary-50 transition-colors no-underline"
            >
              記事一覧を見る
      ...
      {topActions.length > 0 && (
        <section className="mb-5 sm:mb-8 animate-fade-in-up stagger-1">
          <h2 className="text-[13px] font-bold text-gray-700 mb-2.5 flex items-center gap-1.5">
          ...
          <div className="bg-white rounded-lg border border-gray-200/80 divide-y divide-gray-100 overflow-hidden">
            {topActions.map(({ item, articleId, articleTitle }, i) => (
              <Link
                key={i}
                to={`/articles/${articleId}`}
                className="flex items-start gap-2.5 px-3 py-2.5 hover:bg-gray-50/60 transition-colors no-underline group"
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] text-gray-700 leading-snug group-hover:text-primary-700 transition-colors">
                    {item}
                  </p>
                  <p className="text-[11px] text-gray-400 mt-0.5 truncate">{articleTitle}</p>
                </div>
              </Link>
            ))}
          </div>
          ...
          {trending.map((article, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="text-[13px] font-bold text-gray-800 group-hover:text-primary-700 transition-colors leading-snug">
                    {article.title}
                  </h3>
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    <span className="text-[11px] text-gray-300">{article.publishedAt}</span>
                  </div>
                </div>
      ...
      <section className="mb-5 sm:mb-8 animate-fade-in-up stagger-3">
        <h2 className="text-[13px] font-bold text-gray-700 mb-2.5">カテゴリ</h2>
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-[12px] sm:text-[13px] font-bold text-gray-800 group-hover:text-primary-700 transition-colors">
                  {cat}
                </h3>
                <p className="text-[11px] text-gray-400 mt-0.5">{count}件</p>
              </Link>
            );
          })}
        </div>
      ...
      {/* ── 注目記事 ── */}
      <section className="mb-5 sm:mb-8 animate-fade-in-up stagger-4">
        <div className="flex items-center justify-between mb-2.5">
          <h2 className="text-[13px] font-bold text-gray-700">注目記事</h2>
          <Link to="/articles" className="text-[11px] text-primary-600 hover:text-primary-700 no-underline font-medium">
            すべて見る →
          </Link>
        </div>
      ...
      {/* ── 最近の記事 ── */}
      <section className="mb-5 animate-fade-in-up stagger-5">
        <h2 className="text-[13px] font-bold text-gray-700 mb-2.5">最近追加した記事</h2>
        <div className="bg-white rounded-lg border border-gray-200/80 divide-y divide-gray-100 overflow-hidden">
